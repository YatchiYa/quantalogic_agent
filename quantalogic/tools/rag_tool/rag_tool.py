"""RAG Tool using LlamaIndex and ChromaDB for document retrieval and querying.
...
- Flexible response modes for different use cases
"""
...
class RagTool(Tool):
    """RAG tool using LlamaIndex and ChromaDB."""

    name: str = "rag_tool"
    description: str = (
        "RAG tool for document retrieval and QA using LlamaIndex and ChromaDB. "
    ...
    arguments: List[ToolArgument] = [
        ToolArgument(
            ...
            required=True,
            example="What are the key provisions in the civil code?",
        ),
        ToolArgument(
            name="max_sources",
            arg_type="int",
        ...
        ToolArgument(
            name="response_mode",
            arg_type="string",
            description="Response mode: 'sources_only', 'contextual_answer', 'answer_with_sources', or 'direct_answer'",
            required=False,
    ...
    def __init__(
        ...
        logger.info("Initializing RagTool with parameters:")
    ...
    def _generate_contextual_answer(self, query: str, sources: List[Dict]) -> str:
        ...
        Returns:
            Contextual answer based on sources
        """
        if not sources:
    ...
    def execute(
        ...
        Args:
            ...
            response_mode: Response format mode
            
        Returns:
            JSON string with query results or plain text for DIRECT_ANSWER mode
        """
        try:
            if not self.index:
            ...
            # Direct answer mode - returns plain text instead of JSON
            if response_mode == ResponseMode.DIRECT_ANSWER:
                if not results:
                    return "No relevant information found for your query."
                
                # Sort sources by relevance
                sorted_sources = sorted(results, key=lambda x: x["score"], reverse=True)
                
                # Build a comprehensive answer
                answer_parts = [f"# Answer to: {query}\n"]
                
                # Add main content from top sources
                answer_parts.append("## Main Information\n")
                for i, source in enumerate(sorted_sources[:max_sources], 1):
                    answer_parts.append(f"{source['content']}\n")
                
                # Add source references at the end
                answer_parts.append("\n## Sources\n")
                for i, source in enumerate(sorted_sources[:max_sources], 1):
                    answer_parts.append(
                        f"{i}. {source['metadata'].get('file_name', 'Unknown source')} "
                        f"(Relevance: {source['score']:.2f})"
                    )
                
                return "\n".join(answer_parts)
            
            # Standard JSON response modes
            elif response_mode == ResponseMode.SOURCES_ONLY:
