...
class LegalEmbeddingRAG(Tool):
    ...
    def __init__(
        ...
        if use_temp_dir:
            import tempfile
            self.temp_dir = tempfile.mkdtemp(prefix="legal_chroma_")
    ...
    def _create_index(self, documents: List[Document]) -> Optional[VectorStoreIndex]:
        ...
        try:
            ...
            logger.info(f"Creating index with {len(processed_docs)} articles...")
            try:
                ...
                logger.success(f"Successfully created and persisted index with {len(processed_docs)} articles")
                ...
                if not self.use_temp_dir:
                    logger.info("Attempting to create index in temporary directory...")
                    import tempfile
                    temp_dir = tempfile.mkdtemp(prefix="legal_recovery_")
                    try:
                        self.storage_context.persist(persist_dir=temp_dir)
                        shutil.copytree(temp_dir, self.persist_dir, dirs_exist_ok=True)
                        shutil.rmtree(temp_dir)
                        logger.success("Successfully recovered index creation")
                        return index
            ...
            logger.error(f"Error in index creation: {str(e)}")
            return None
...
if __name__ == "__main__":
    ...
    try:
        ...
        tool = LegalEmbeddingRAG(
            ...
            chunk_size=512,
            chunk_overlap=128,
            force_reindex=True,  # Force reindex to test the changes
        ...
        try:
            # Test each response mode
            for mode in ResponseMode:
                logger.info(f"\nTesting response mode: {mode.value}")
                result = tool.execute(
                    query=test_query,
                    max_sources=5,
                    min_relevance=0.5,
