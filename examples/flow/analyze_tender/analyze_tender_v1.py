...
# Node: Analyze Detailed Budget
@Nodes.structured_llm_node(
    system_prompt="You are a financial expert specializing in detailed budget analysis for tenders.",
    output="budget_analysis",
    response_model=DetailedBudgetAnalysis,
    prompt_template="""
Perform a detailed budget analysis for this tender:
...
2. Cost assumptions and justifications
3. Contingency recommendations
4. Cost risks and mitigation strategies
5. Budget optimization opportunities
"""
)
async def analyze_detailed_budget(
    financial_analysis: FinancialRequirement,
    tender_info: TenderInfo,
    document_content: str
) -> DetailedBudgetAnalysis:
    """Generate detailed budget analysis and breakdown."""
    pass
...
def create_tender_analysis_workflow() -> Workflow:
    ...
    wf.node("analyze_detailed_budget", inputs_mapping={"model": "analysis_model"})
    ...
    wf.transitions["generate_detailed_analysis"] = [("analyze_detailed_budget", None)]
    wf.transitions["analyze_detailed_budget"] = [("analyze_project_timeline", None)]
