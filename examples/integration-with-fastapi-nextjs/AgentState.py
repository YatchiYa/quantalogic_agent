...
from .models import AnalyzePaperRequest, BookNovelRequest, CompanyAnalyzeRequest, CompetitiveContentRequest, ContractRevisionRequest, ConvertRequest, CourseRequest, CSVExplorerRequest, DefendLegalCaseRequest, DocumentContext, EventMessage, EuromillionsPredictionRequest, FacebookContentRequest, GitAnalyzeRequest, ImageAnalysisRequest, ImageGenerationRequest, InstagramContentRequest, JourneyRequest, LinkedInContentRequest, LinkedInIntroduceContentRequest, MediumContentRequest, QuizRequest, StarterPackRequest, TalkWithDocumentRequest, ToolConfig, ToolParameters, TopicAnalyzeRequest, TutorialRequest, TwitterContentRequest, UserValidationRequest, UserValidationResponse, AgentConfig, TaskSubmission


from .qapps.talk_with_document import documentRagModal
from .qapps.defender_legal_expert import defendLegalCaseModal
from .qapps.contract_revision import contractRevisionModal
...
class AgentState:
    ...
    async def stop_task(self, task_id: str) -> bool:
        ...
        Returns:
            bool: True if task was stopped successfully, False if task was not found or not running
        """
        ...
        if task_info["status"] != "running":
            logger.info(f"Task {task_id} is not running (status: {task_info['status']}), cannot stop")
            return False
    ...
    async def execute_facebook_content(self, task_id: str, request: FacebookContentRequest) -> None:
        """Execute an image analysis task asynchronously."""
        if task_id not in self.tasks:
            raise ValueError(f"Task {task_id} not found")
        ...
        try:
            ...
            from flows.facebook_content.facebook_content_flow import generate_facebook_content
            ...
            result = await loop.run_in_executor(
                None,
                lambda: asyncio.run(generate_facebook_content(  
            ...
            logger.debug(f"================================================================")
            logger.info(f"================================================================")
            logger.info(f"== Facebook content generation result: {result}")
            ...
            self._handle_event("task_solve_end", {
                "task_id": task_id, 
                "agent_id": "default",
                "message": "Facebook content generation completed",
                "result": result
            })
            ...
            self._handle_event("error_tool_execution", {
                "task_id": task_id,
                "message": f"Facebook content generation failed: {str(e)}"
            })
            
            logger.exception(f"Error generating Facebook content {task_id}")
        finally:
            self.remove_task_event_queue(task_id) 
    ...
    async def execute_instagram_content(self, task_id: str, request: InstagramContentRequest) -> None:
        """Execute an image analysis task asynchronously."""
        if task_id not in self.tasks:
            raise ValueError(f"Task {task_id} not found")
        ...
        try:
            ...
            result = await loop.run_in_executor(
                None,
                lambda: asyncio.run(generate_instagram_content(  
                    content_context=request.content,
                    num_images=request.num_images,
                    generate_images=request.generate_images,
                    analysis_model=request.analysis_model,
            ...
            logger.debug(f"================================================================")
            logger.info(f"================================================================")
            logger.info(f"== Instagram content generation result: {result}")
            ...
            self._handle_event("task_solve_end", {
                "task_id": task_id, 
                "agent_id": "default",
                "message": "Instagram content generation completed",
                "result": result
            })
            ...
            self._handle_event("error_tool_execution", {
                "task_id": task_id,
                "message": f"Instagram content generation failed: {str(e)}"
            })
            
            logger.exception(f"Error generating Instagram content {task_id}")
        finally:
            self.remove_task_event_queue(task_id) 
    ...
    async def execute_linkedin_content(self, task_id: str, request: LinkedInContentRequest) -> None:
        """Execute an image analysis task asynchronously."""
        if task_id not in self.tasks:
            raise ValueError(f"Task {task_id} not found")
        ...
        try:
            ...
            from flows.linkedin_content.linkedin_content_flow import generate_linkedin_content
            ...
            result = await loop.run_in_executor(
                None,
                lambda: asyncio.run(generate_linkedin_content(  
                    content_context=request.content,
                    num_images=request.num_images,
                    generate_images=request.generate_images,
                    analysis_model=request.analysis_model,
            ...
            logger.debug(f"================================================================")
            logger.info(f"================================================================")
            logger.info(f"== LinkedIn content generation result: {result}")
            ...
            self._handle_event("task_solve_end", {
                "task_id": task_id, 
                "agent_id": "default",
                "message": "LinkedIn content generation completed",
                "result": result
            })
            ...
            self._handle_event("error_tool_execution", {
                "task_id": task_id,
                "message": f"LinkedIn content generation failed: {str(e)}"
            })
            
            logger.exception(f"Error generating LinkedIn content {task_id}")
        finally:
            self.remove_task_event_queue(task_id) 
    ...
    async def execute_medium_content(self, task_id: str, request: MediumContentRequest) -> None:
        """Execute an image analysis task asynchronously."""
        if task_id not in self.tasks:
            raise ValueError(f"Task {task_id} not found")
        ...
        try:
            ...
            from flows.medium_content.medium_content_flow import generate_medium_content
            ...
            result = await loop.run_in_executor(
                None,
                lambda: asyncio.run(generate_medium_content(  
                    content_context=request.content,
                    num_images=request.num_images,
                    generate_images=request.generate_images,
                    analysis_model=request.analysis_model,
            ...
            logger.debug(f"================================================================")
            logger.info(f"================================================================")
            logger.info(f"== Medium content generation result: {result}")
            ...
            self._handle_event("task_solve_end", {
                "task_id": task_id, 
                "agent_id": "default",
                "message": "Medium content generation completed",
                "result": result
            })
            ...
            self._handle_event("error_tool_execution", {
                "task_id": task_id,
                "message": f"Medium content generation failed: {str(e)}"
            })
            
            logger.exception(f"Error generating Medium content {task_id}")
        finally:
            self.remove_task_event_queue(task_id) 
    ...
    async def execute_competitive_content(self, task_id: str, request: CompetitiveContentRequest) -> None:
        """Execute a competitive analysis task asynchronously."""
        if task_id not in self.tasks:
            raise ValueError(f"Task {task_id} not found")
        ...
        try:
            logger.info(f"== Starting competitive analysis: {task_id}") 
            
            # Add the examples directory to Python path
            examples_dir = os.path.abspath(os.path.join(os.path.dirname(__file__), '..', '..', 'examples', "integration-with-fastapi-nextjs"))
            sys.path.append(examples_dir)
            
            from flows.competitive_intel.competitive_flow import analyze_competition
            ...
            result = await loop.run_in_executor(
                None,
                lambda: asyncio.run(analyze_competition(  
            ...
            logger.exception(f"Error generating competitive analysis {task_id}")
        finally:
            self.remove_task_event_queue(task_id) 
    ...
    async def execute_git_analyze(self, task_id: str, request: GitAnalyzeRequest) -> None:
        """Execute a Git repository analysis task asynchronously."""
        if task_id not in self.tasks:
            raise ValueError(f"Task {task_id} not found")
        ...
        try:
            logger.info(f"== Starting Git repository analysis: {task_id}") 
            
            # Add the examples directory to Python path
            examples_dir = os.path.abspath(os.path.join(os.path.dirname(__file__), '..', '..', 'examples', "integration-with-fastapi-nextjs"))
            sys.path.append(examples_dir)
            
            from flows.git_inspect.repo_analysis_flow import analyze_repository_flow
            
            # Run repository analysis in a thread to not block the event loop
            loop = asyncio.get_running_loop()
            ...
            # Run the analyze_repository_flow function directly in the executor
            # since it's not a coroutine but a regular function
            result = await loop.run_in_executor(
                None,
                lambda: analyze_repository_flow(
            ...
            logger.debug(f"================================================================")
            logger.info(f"================================================================")
            logger.info(f"== Git repository analysis result: {result}")
            ...
            self._handle_event("task_solve_end", {
                "task_id": task_id, 
                "agent_id": "default",
                "message": "Git repository analysis completed",
                "result": result
            })
            ...
            self._handle_event("error_tool_execution", {
                "task_id": task_id,
                "message": f"Git repository analysis failed: {str(e)}"
            })
            
            logger.exception(f"Error analyzing Git repository {task_id}")
        finally:
            self.remove_task_event_queue(task_id)

    async def execute_twitter_content(self, task_id: str, request: TwitterContentRequest) -> None:
        """Execute a Twitter content generation task asynchronously."""
        if task_id not in self.tasks:
            raise ValueError(f"Task {task_id} not found")
        ...
        try:
            logger.info(f"== Starting Twitter content generation: {task_id}") 
            
            # Add the examples directory to Python path
            examples_dir = os.path.abspath(os.path.join(os.path.dirname(__file__), '..', '..', 'examples', "integration-with-fastapi-nextjs"))
            sys.path.append(examples_dir)
            
            from flows.twitter_content.twitter_content_flow import generate_twitter_content
            
            # Run content generation in a thread to not block the event loop
            loop = asyncio.get_running_loop()
            ...
            result = await loop.run_in_executor(
                None,
                lambda: asyncio.run(generate_twitter_content(  
                    content_context=request.content,
                    num_images=request.num_images,
                    generate_images=request.generate_images,
                    analysis_model=request.analysis_model,
            ...
            logger.info(f"== Twitter content generation result: {result}")
            ...
            self._handle_event("task_solve_end", {
                "task_id": task_id, 
                "agent_id": "default",
                "message": "Twitter content generation completed",
                "result": result
            })
            ...
            self._handle_event("error_tool_execution", {
                "task_id": task_id,
                "message": f"Twitter content generation failed: {str(e)}"
            })
            
            logger.exception(f"Error generating Twitter content {task_id}")
        finally:
            self.remove_task_event_queue(task_id)

    async def execute_euromillions_prediction(self, task_id: str, request: EuromillionsPredictionRequest) -> None:
        """Execute a Euromillions prediction task asynchronously."""
        if task_id not in self.tasks:
            raise ValueError(f"Task {task_id} not found")
        ...
        try:
            logger.info(f"== Starting Euromillions prediction: {task_id}") 
            
            # Add the examples directory to Python path
            examples_dir = os.path.abspath(os.path.join(os.path.dirname(__file__), '..', '..', 'examples', "integration-with-fastapi-nextjs"))
            sys.path.append(examples_dir)
            
            from flows.euromillion.euromillions_prediction_flow import run_workflow
            
            # Run prediction in a thread to not block the event loop
            loop = asyncio.get_running_loop()
            ...
            logger.info(f"== Euromillions prediction result: {result}")
            ...
            saved_prediction = result.get("saved_prediction")
            analysis_saved = result.get("analysis_saved")
            top_combinations = result.get("top_combinations")
            top_combinations_saved = result.get("top_combinations_saved")
            ...
            response = {
                ...
                "saved_files": {
                    ...
                    "top_combinations": top_combinations_saved
                },
                "top_combinations": top_combinations
            }

            self._update_task_success(task_info, response, None) 
            
            # Create event for task completion
            self._handle_event("task_solve_end", {
                "task_id": task_id, 
                "agent_id": "default",
                "message": "Euromillions prediction completed",
                "result": response
            })
            ...
            self._handle_event("error_tool_execution", {
                "task_id": task_id,
                "message": f"Euromillions prediction failed: {str(e)}"
            })
            
            logger.exception(f"Error generating Euromillions prediction {task_id}")
        finally:
            self.remove_task_event_queue(task_id)

    async def execute_starter_pack(self, task_id: str, request: StarterPackRequest) -> None:
        """Execute a starter pack generation task asynchronously."""
        if task_id not in self.tasks:
            raise ValueError(f"Task {task_id} not found")
        ...
        try:
            logger.info(f"== Starting starter pack generation: {task_id}") 
            
            # Add the examples directory to Python path
            examples_dir = os.path.abspath(os.path.join(os.path.dirname(__file__), '..', '..', 'examples', "integration-with-fastapi-nextjs"))
            sys.path.append(examples_dir)
            
            from flows.starter_pack.starter_pack_flow import generate_starter_pack_workflow
            ...
            result = await loop.run_in_executor(
                None,
                lambda: asyncio.run(generate_starter_pack_workflow(  
                    reference_image_url=request.reference_image_url,
                    packaging_style=request.packaging_style,
                    vision_model=request.vision_model,
            ...
            logger.info(f"== Starter pack generation result: {result}")
            ...
            self._handle_event("task_solve_end", {
                "task_id": task_id, 
                "agent_id": "default",
                "message": "Starter pack generation completed",
                "result": response
            })
            ...
            self._handle_event("error_tool_execution", {
                "task_id": task_id,
                "message": f"Starter pack generation failed: {str(e)}"
            })
            
            logger.exception(f"Error generating starter pack {task_id}")
        finally:
            self.remove_task_event_queue(task_id)

    async def execute_csv_explorer(self, task_id: str, request: CSVExplorerRequest) -> None:
        """Execute a CSV explorer task asynchronously."""
        if task_id not in self.tasks:
            raise ValueError(f"Task {task_id} not found")
        ...
        try:
            logger.info(f"== Starting CSV explorer: {task_id}") 
            
            # Add the examples directory to Python path
            examples_dir = os.path.abspath(os.path.join(os.path.dirname(__file__), '..', '..', 'examples', "integration-with-fastapi-nextjs"))
            sys.path.append(examples_dir)
            
            from flows.csv_explorer.csv_explorer import main
            ...
            logger.info(f"== CSV exploration result: {result}")

            # Extract key information for the response
            report_path = result.get("report_path", "")
            visualization_results = result.get("visualization_results", [])
            
            # Create a clean response object with visualization paths
            viz_paths = []
            for viz in visualization_results:
                if viz.get("success", False) and "file_path" in viz:
                    viz_paths.append({
                        "title": viz.get("title", "Visualization"),
                        "description": viz.get("description", ""),
                        "path": viz.get("file_path", "")
                    })
            ...
            self._handle_event("task_solve_end", {
                "task_id": task_id, 
                "agent_id": "default",
                "message": "CSV exploration completed",
                "result": response
            })
            ...
            self._handle_event("error_tool_execution", {
                "task_id": task_id,
                "message": f"CSV exploration failed: {str(e)}"
            })
            
            logger.exception(f"Error during CSV exploration {task_id}")
        finally:
            self.remove_task_event_queue(task_id)

    async def execute_company_analyzer(self, task_id: str, request: CompanyAnalyzeRequest) -> None:
        """Execute a company analysis task asynchronously."""
        if task_id not in self.tasks:
            raise ValueError(f"Task {task_id} not found")
        ...
        try:
            logger.info(f"== Starting company analysis: {task_id}") 
            
            # Add the examples directory to Python path
            examples_dir = os.path.abspath(os.path.join(os.path.dirname(__file__), '..', '..', 'examples', "integration-with-fastapi-nextjs"))
            sys.path.append(examples_dir)
            
            from flows.company_analyzer.company_analyzer import analyze_company
            ...
            result = await loop.run_in_executor(
                None,
                lambda: asyncio.run(analyze_company(
                    company_name=request.company_name,
            ...
            self._handle_event("error_tool_execution", {
                "task_id": task_id,
                "message": f"Company analysis failed: {str(e)}"
            })
            
            logger.exception(f"Error analyzing company {task_id}")
        finally:
            self.remove_task_event_queue(task_id)

    async def execute_topic_analyzer(self, task_id: str, request: TopicAnalyzeRequest) -> None:
        """Execute a topic analysis task asynchronously."""
        if task_id not in self.tasks:
            raise ValueError(f"Task {task_id} not found")
        ...
        try:
            logger.info(f"== Starting topic analysis: {task_id}") 
            
            # Add the examples directory to Python path
            examples_dir = os.path.abspath(os.path.join(os.path.dirname(__file__), '..', '..', 'examples', "integration-with-fastapi-nextjs"))
            sys.path.append(examples_dir)
            
            from flows.topic_analysis.topic_analysis import analyze_topic
            ...
            self._handle_event("task_solve_start", {
                "task_id": task_id, 
                "agent_id": "default",
                "message": "Topic analysis started"
            }) 

            result = await loop.run_in_executor(
                None,
                lambda: asyncio.run(analyze_topic(
            ...
            logger.debug(f"================================================================")
            logger.info(f"================================================================")
            logger.info(f"== Topic analysis result: {result}")
            ...
            logger.exception(f"Error analyzing topic {task_id}")
        finally:
            self.remove_task_event_queue(task_id)



    async def execute_context_analysis(self, task_id: str, request: DocumentContext) -> None:
        """Execute a topic analysis task asynchronously."""
        if task_id not in self.tasks:
            raise ValueError(f"Task {task_id} not found")
        ...
        try:
            logger.info(f"== Starting context analysis: {task_id}") 
            
            # Add the examples directory to Python path
            examples_dir = os.path.abspath(os.path.join(os.path.dirname(__file__), '..', '..', 'examples', "integration-with-fastapi-nextjs"))
            sys.path.append(examples_dir)
            
            from flows.document_analyzer_output_csv.document_analysis_flow_with_context import analyze_context
            ...
            result = await loop.run_in_executor(
                None,
                lambda: asyncio.run(analyze_context(
            ...
            logger.exception(f"Error analyzing context {task_id}")
        finally:
            self.remove_task_event_queue(task_id)



    async def execute_talk_with_document(self, task_id: str, request: TalkWithDocumentRequest) -> None:
        """Execute a talk with document task asynchronously."""
        await documentRagModal(self, task_id, request)

    async def execute_defend_legal_case(self, task_id: str, request: DefendLegalCaseRequest) -> None:
        """Execute a defend legal case task asynchronously."""
        await defendLegalCaseModal(self, task_id, request)

    async def execute_contract_revision(self, task_id: str, request: ContractRevisionRequest) -> None:
        """Execute a contract revision task asynchronously."""
        await contractRevisionModal(self, task_id, request)