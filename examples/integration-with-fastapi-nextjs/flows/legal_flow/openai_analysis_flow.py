...
from quantalogic.tools.rag_tool.simple_rag_tool import SimpleRagTool


# Initialize Typer app and rich console
app = typer.Typer(help="Analyze InRag documents using SimpleRagTool")
console = Console()
...
async def initialize_rag_tool(
    ...
    """Initialize the SimpleRagTool tool."""
    try:
...
def analyze(
    ...
    chunk_size: Annotated[int, typer.Option(help="Size of text chunks")] = 1024,
    chunk_overlap: Annotated[int, typer.Option(help="Overlap between chunks")] = 256,
    ...
    llm_model: Annotated[str, typer.Option(help="OpenAI LLM model name")] = "gpt-4o-mini"
):
    """Analyze InRag documents using SimpleRagTool workflow."""
    try:
        # Use home directory if persist_dir is None
        if persist_dir is None:
            home_dir = os.path.expanduser("~")
