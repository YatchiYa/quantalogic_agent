...
async def stop_task_caller(task_id: str) -> Dict[str, str]:
    """Stop a running task."""
    if task_id not in agent_state.tasks:
        raise HTTPException(status_code=404, detail="Task not found")
    
    success = await agent_state.stop_task(task_id)
    
    if not success:
        raise HTTPException(status_code=400, detail="Task could not be stopped (not running)")
    
    return {"status": "stopped", "task_id": task_id, "message": "Task stopped successfully"}
